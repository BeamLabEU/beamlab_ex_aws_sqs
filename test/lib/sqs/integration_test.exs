defmodule ExAws.SQSIntegrationTest do
  ...
  test "change_message_visibility/3", context do
    ...
    receipt_handle = List.first(messages)["ReceiptHandle"]
  ...
  test "change_message_visibility_batch/2", context do
    SQS.send_message_batch(
      context.queue_url,
      [
        [id: "message_1", message_body: "This is the first message body."],
        [id: "message_2", message_body: "This is the second message body."]
      ]
    )
    |> ExAws.request()
    ...
    assert {:ok, %{"Successful" => successful}} =
             SQS.change_message_visibility_batch(context.queue_url, messages_payload)
             |> ExAws.request()

    assert Enum.sort(successful) == Enum.map(messages, & &1["MessageId"]) |> Enum.sort()
  end

  test "create_queue/1" do
    queue_name = "test_queue_2"

    assert {:ok, %{"QueueUrl" => queue_url}} = SQS.create_queue(queue_name) |> ExAws.request()
  ...
  test "delete_message/2", context do
    ...
    {:ok, %{"Messages" => messages}} = SQS.receive_message(context.queue_url) |> ExAws.request()

    refute Enum.empty?(messages)
    [msg | _] = messages

    assert {:ok, _} =
             SQS.delete_message(context.queue_url, msg["ReceiptHandle"]) |> ExAws.request()
  end
  ...
  test "delete_message_batch/2", context do
    ...
    assert {:ok, _} =
             SQS.delete_message_batch(context.queue_url, [
               %{id: "message_1", receipt_handle: msg1["ReceiptHandle"]},
               %{id: "message_2", receipt_handle: msg2["ReceiptHandle"]}
             ])
             |> ExAws.request()
  end

  test "delete_queue/1" do
    {:ok, %{"QueueUrl" => queue_url}} = SQS.create_queue("test_queue_4") |> ExAws.request()

    assert {:ok, _} = SQS.delete_queue(queue_url) |> ExAws.request()
  end

  test "get_queue_attributes/1", context do
    assert {:ok, %{"Attributes" => _}} =
  ...
  test "get_queue_url/1" do
    assert {:ok, %{"QueueUrl" => queue_url}} = SQS.get_queue_url(@queue_name) |> ExAws.request()
  ...
  test "get_queue_url/2" do
    assert {:ok, %{"QueueUrl" => queue_url}} =
             SQS.get_queue_url(@queue_name, queue_owner_aws_account_id: "foo")
             |> ExAws.request()
  ...
  test "list_queues/0" do
    assert {:ok, %{"QueueUrls" => queues}} = SQS.list_queues() |> ExAws.request()
    assert is_list(queues)
  end

  test "list_queues/1" do
    assert {:ok, response} = SQS.list_queues(queue_name_prefix: "prefix") |> ExAws.request()
    assert is_list(Map.get(response, "QueueUrls", []))
  end

  test "list_queue_tags/1", context do
    assert {:ok, %{"Tags" => tags}} = SQS.list_queue_tags(context.queue_url) |> ExAws.request()
  ...
  test "receive_message/1", context do
    assert {:ok, response} = SQS.receive_message(context.queue_url) |> ExAws.request()
    assert is_list(Map.get(response, "Messages", []))
  end
  ...
  test "send_message/3", context do
    assert {:ok, %{"MD5OfMessageAttributes" => _, "MD5OfMessageBody" => _, "MessageId" => _}} =
             SQS.send_message(
               context.queue_url,
  ...
  test "send_message_batch/2", context do
      ...
      SQS.send_message_batch(
        ...
        [
          ...
          [
            ...
            message_attributes: [
              %{name: "TestAnotherStringAttribute", data_type: :string, value: "still testing!"}
            ]
          ]
        ]
      )
    ...
    assert ["test_message_1", "test_message_2"] ==
             successful |> Enum.map(& &1["Id"]) |> Enum.sort()
  end
